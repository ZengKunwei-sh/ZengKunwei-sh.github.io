...
- [ ] 运行结束后将结果记录在日志中
## 模拟账户登录
&emsp;学习强国的网页版需要使用手机扫描二维码登录账户。由此我们可以在手动登录账户后获取页面cookies，将cookies保存为文本文件，使得之后都可以免去登录步骤。
...
browser.implicitly_wait(30)                                             # 设置等待时间，利用该时间手机扫码登录账户
browser.find_element(By.XPATH, '//div[@class="top-div"]')               # 当能识别到该元素时，可以确认登录账户成功
```
### 将cookies存储为文本文件
```
...
```
### 遇到的问题
隔了一天之后，前一天保存的cookies却无法使用，不知道是为什么。

这导致在每天登录的时候都需要手动扫码，太麻烦了，自动化程度不高。
...
<img src="/images/2022-3-19-学习强国积分自动化/专项答题.png" width="600" height="250"/>

`find_element`方法不知道为什么一直无法识别`@class="item"`的元素，不知道是不是因为`flex`标签的问题。

最后只好放弃关于“专项答题”这一板块的自动化。
